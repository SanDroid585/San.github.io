...
void keyPressed () { /* Simple Play
  ...
  if ( key=='F' || key=='f' ) playList[currentSong].skip( 5000 ); // Fast Forward, Rewind, & Play Again //Parameter: milliseconds
  if ( key=='R' || key=='r' ) playList[currentSong].skip( -5000 ); // Fast Reverse & Play //Parameter: negative numbers
  if ( key=='W' || key=='w' ) { // MUTE
    //
  ...
  if ( playList[currentSong].isPlaying() ) {
    ...
    // PREVIOUS will not automatically play the song
    //song[currentSong].play();
  }
}
